...
class TanhGaussianPolicy(Mlp, ExplorationPolicy):
    ...
    def forward(self, obs, reparameterize=True, deterministic=False, return_log_prob=False, num_repeat: int = 1):
        h = obs
        if num_repeat > 1:
            # (B, F) -> (B, 1, F) -> (B, num_actions, F) -> (B * num_actions, F)
            h = h.unsqueeze(1).repeat(1, num_repeat, 1).view(h.shape[0] * num_repeat, h.shape[1])
        for i, fc in enumerate(self.fcs):
            h = self.hidden_activation(fc(h))
        mean = self.last_fc(h)
        if self.std is None:
...
if __name__ == "__main__":
    ...
    obs_dim = 792
    act_dim = 2
    policy = TanhGaussianPolicy(hidden_size, obs_dim, act_dim)
